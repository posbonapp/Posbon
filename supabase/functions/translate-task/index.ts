...
Deno.serve(async (req) => {
  try {
    ...
    // Avoid re-translating on unrelated updates (status/assigned_to/...)
    ...
    // The "new task assigned" push must carry the translation, so we trigger
    // notify-task ourselves right after translation completes (or fails)
    // instead of racing it via a parallel INSERT webhook. notify-task itself
    // still decides whether to actually send (it no-ops without an
    // assignee) — the notify-task-hook webhook should keep only its Update
    // event (for redo/done), with Insert removed/disabled.
    let notifyStatus: number | null = null
    if (!oldRecord) {
      const res = await fetch(`${supabaseUrl}/functions/v1/notify-task`, {
        method: 'POST',
